...
class SoftmaxCrossEntropy:
    ...
    def forward(self, y, y_hat):
        ...
        exponent=y_hat-np.max(y_hat, axis=1, keepdims=True)
        softmax=np.exp(exponent)/np.sum(np.exp(exponent), axis=1, keepdims=True)
        self.y_grad_placeholder=softmax-y
        self.loss_val=-np.sum(np.multiply(np.log(softmax), y), axis=1)


    def backward(self):
        # You can call autograd's backward here OR in the mlp.
        return self.y_grad_placeholder
