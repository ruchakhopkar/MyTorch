...
class Autograd():
    ...
    def backward(self, divergence):
        ...
        for i in range(len(self.operation_list)-1, -1, -1):
            
            if i==len(self.operation_list)-1:
                grad_of_output=divergence
            else:
                grad_of_output=self.memory_buffer.get_param(self.operation_list[i].output)
            
            
            grad_a, grad_b=self.operation_list[i].backward_operation(grad_of_output, self.operation_list[i].inputs[0]
                                                                ,self.operation_list[i].inputs[1])
            gradients=[grad_a, grad_b]
            
            for j in range(len(self.operation_list[i].inputs)):
                
                self.memory_buffer.set_param(self.operation_list[i].inputs[j],gradients[j])
                
                self.memory_buffer.update_param(self.operation_list[i].inputs[j],self.operation_list[i].gradients_to_update[j])
